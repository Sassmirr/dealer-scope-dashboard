...
def manual_sarimax(series, steps=3):
    """Manual SARIMAX fallback if auto_arima fails."""
    p = d = q = range(0, 2)
    P = D = Q = range(0, 2)
...
def predict_sarimax_auto(df, steps=3):
    ...
    for kpi, group in df.groupby("english_name"):
        ...
        if pred is None:
            try:
                pred = manual_sarimax(series, steps)
                if pred is not None:
...
if __name__ == "__main__":
    ...
    # Save to CSV
    forecast_df.to_csv("src/outputs/sarimax_forecast.csv", index=False)
    print("Forecast saved to src/outputs/sarimax_forecast.csv")
